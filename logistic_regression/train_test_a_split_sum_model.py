...
Models are trained using an 80/20 train-test split with features representing partial sums of the power spectrum
(denoted in commenting below as "A" and "B") and the second virial coefficient, $B_{22}$ (denoted in commenting below as "F").
Baseline performance is established using the simpler "C-F" model, where C represents the total variance (sum of "A" and "B").
...
def load_combination_mapping(model_feature_mapping, entry_index):
    """
    Reads the JSON file, extracts the (entry_index)-th combination.
    The JSON file maps feature combinations to combination names.
    Each key is a tuple of three .npy file paths:
     - partial sum of the power spectrum up to a certain wavenumber (e.g., "power_up_to_include_0_1.npy")
     - partial sum of the power spectrum for modes above that wavenumber (e.g., "power_above_0_1_power.npy")
     - the B2 value (e.g., "b2.npy")
    Value is the combination name (str) (e.g., "power-split-0-1").

    Returns: combination_str (str) representing the tuple of feature array (.npy) file paths and combo_name (str)
    representing name associated with the feature combination (e.g., "power-split-0-1").
    """
    with open(model_feature_mapping, 'r') as json_file:
...
def get_next_trial_index(specific_model_results_dir):
    existing_files = glob(os.path.join(specific_model_results_dir, 'roc_data_trial_*.json'))
...
def evaluate_model_performance(model_name, model, X_train, y_train, X_test, y_test):
    ...
    Params:
        - model_name (str): Type of model being evaluated
    ...
    Returns: dict containing AUC scores, model coefficients, intercept, 
                predicted labels, and predicted probabilities for both training 
                and test sets.
    """
...
def run_trial(npy_files, seq_ps_file, specific_model_results_dir, trial_index, random_state, gamma_factor):
    """
    ...
    # print("Baseline - [CF model] AUC train:", results["auc_cf_train"], "AUC test:", results["auc_cf_test"])
    ...
    roc_abf = evaluate_model_performance("Model_ABF", results["model_abf"], results["X_train_abf"], y_train, results["X_test_abf"], y_test)
    roc_cf = evaluate_model_performance("Model_CF", results["model_cf"], results["X_train_cf"], y_train, results["X_test_cf"], y_test)
    roc_derived = evaluate_model_performance("Derived_ABF", results["derived_model_abf"], results["X_train_abf"], y_train, results["X_test_abf"], y_test)
    ...
    all_evaluation_details["best_training_model"] = best_model
    all_evaluation_details["best_test_auc"] = roc_derived["test_auc"] if best_model == "Derived_ABF" else results["auc_abf_test"]

    print("Writing model evaluation details...")
    output_file = f"{specific_model_results_dir}/roc_data_trial_{trial_index}.json"
    with open(output_file, "w") as f:
        json.dump(all_evaluation_details, f, indent=2)
    ...
    Best Test AUC: {all_evaluation_details["best_test_auc"]}
    """
    summary_file = f"{specific_model_results_dir}/summary_trial_{trial_index}.txt"
...
if __name__ == "__main__":
   ...
   # Use provided JSON file path and load its specified model-feature mapping
    ...
    combination_str, combo_name = load_combination_mapping(model_feature_mapping_file, args.entry_index)
    npy_files = get_npy_files(combination_str, args.feature_data_dir)

    specific_model_results_dir = os.path.join(args.model_results_dir, combo_name)

    # get next trial index and run trial
    trial_index = get_next_trial_index(specific_model_results_dir)
    run_trial(npy_files, args.seq_ps_file, specific_model_results_dir, trial_index, args.random_state, args.gamma_factor)

...
Routines include iteratively adding Fourier modes in order of increasing wavenumber to compute partial sums of the power spectrum,
approximating the contact map via the inverse RFFT2, and evaluating explained variance at each step relative to a null model.
Additionally, the script computes the mean-squared error (MSE) between two explained variance (EV) curves:
one ordered by increasing wavenumber and the other by descending power. This 'divergence metric' quantifies
how variance is distributed across modes—small values indicate concentration in low-wavenumber modes,
while large values suggest significant high-wavenumber contributions.
Saves MSE values and cumulative powers to output files.
"""
...
def compute_explained_variance(original_map, reconstructed_map):
    """Compute explained variance of the reconstructed map relative to the original."""
    frob_norm_diff = np.linalg.norm(original_map - reconstructed_map, 'fro')
    frob_norm_original = np.linalg.norm(original_map, 'fro')
    explained_variance = 1 - np.abs(frob_norm_diff / frob_norm_original)
    return explained_variance

def shuffle_contact_map(original_map):
    """Shuffle a contact map while maintaining its shape."""
    flattened = original_map.flatten()
    np.random.shuffle(flattened)
    return flattened.reshape(original_map.shape)

def compute_explained_variance_null(original_map, num_shuffles=2500):
    """Compute the mean explained variance from shuffled contact maps."""
    total_explained_variance = 0.0

    for _ in range(num_shuffles):
        shuffled_map = shuffle_contact_map(original_map)
        null_explained_variance = compute_explained_variance(original_map, shuffled_map)
        total_explained_variance += null_explained_variance

    return total_explained_variance / num_shuffles

def compute_relative_explained_variance(original_map, reconstructed_map, mean_null_variance):
    """Compute the relative explained variance, correcting for the null model."""
    explained_var = compute_explained_variance(original_map, reconstructed_map)
    relative_explained_var = (explained_var - mean_null_variance) / (1 - mean_null_variance)
    return relative_explained_var
...
def reconstruct_full_fft2_from_rfft2(full_fourier_transform, rfft2_matrix, original_matrix, sorted_unique_indices, mse_output_path):
    """Reconstruct the full FFT matrix from rfft2 output using Hermitian symmetry and diagonal symmetry.
    Also computes MSE beween relative explained variance for increasing wavenumber (k) and descending power on the same x-axis
    ...
    mean_null_variance = compute_explained_variance_null(original_matrix)
    ...
    mse_list = []  # MSE between partial reconstructions and original
    relative_explained_variances = []  # Relative explained variances
    ...
    for count, (row, col) in enumerate(ordered_dict_unique_indices_and_mappings.keys()):
        ...
        # Compute relative explained variance
        relative_explained_variance = (
            0.0 if (row == 0 and col == 0) else compute_relative_explained_variance(original_matrix, partial_reconstruction_with_mean, mean_null_variance)
        )
        relative_explained_variances.append(relative_explained_variance)
        ...
        if count == len(ordered_dict_unique_indices_and_mappings) - 1:
            print(f"Final mode {count}: MSE = {mse:.6f}")
    ...
    used_indices = set()
    relative_explained_variances_sorted_by_power = []
    mean_explained_variance_null = compute_explained_variance_null(original_matrix)
    ...
    for mode, magnitude, redundant_indices in sorted_modes_by_power:
        ...
        # Compute the partial reconstruction and explained variance
        partial_reconstruction = np.real(ifft2(temp_cumulative_fourier_matrix)) + np.mean(original_matrix)
        relative_explained_variance = compute_relative_explained_variance(original_matrix, partial_reconstruction, mean_explained_variance_null)
        relative_explained_variances_sorted_by_power.append(relative_explained_variance)

    # mse bween explained variance (increasing k) and explained variance (descending power) curves
    mse_between_curves = np.mean((np.array(relative_explained_variances) - np.array(relative_explained_variances_sorted_by_power))**2)
    # print("mse between explained variance curves: ", mse_between_curves)
...
def plot_explained_variance(k_list, relative_explained_variances):
    """Plot relative explained variance as a function of wavenumber k."""
    fig, ax = plt.subplots(figsize=(6.0, 4.5))
    
    # Plot explained variance
    ax.plot(k_list, relative_explained_variances, marker='o', linestyle='-', color='blue', linewidth=1.7, markersize=6, label="Explained Variance")
    ax.scatter(k_list, relative_explained_variances, color='blue', edgecolor='#B0C4DE', s=60, zorder=3, marker='o', linewidths=0.5)
    ...
    ax.set_xlabel(r'wavenumber $k$', fontsize=16)
    ax.set_ylabel('Relative explained variance', fontsize=16)
    ax.set_title(r'Relative explained variance vs. $k$', fontsize=16)
...
def plot_explained_variance_comparison(relative_explained_variances, relative_explained_variances_sorted_by_power, mse_between_curves):
    """
    Plot explained variance for increasing wavenumber (k) and descending power on the same x-axis (mode index)
    """
    #print("relative_explained_variances: ", relative_explained_variances)
    #print("relative_explained_variances_sorted_by_power: ", relative_explained_variances_sorted_by_power)
    assert len(relative_explained_variances) == len(relative_explained_variances_sorted_by_power), "Lists must be same length"
    
    fig, ax = plt.subplots(figsize=(6.5, 4.5))
    indices = list(range(1, len(relative_explained_variances) + 1))

    # Plot explained variance vs. increasing wavenumber (k)
    ax.plot(indices, relative_explained_variances, marker='o', linestyle='-', color='blue', linewidth=1.7, markersize=6, label='Increasing $k$')
    ax.scatter(indices, relative_explained_variances, color='blue', edgecolor='#B0C4DE', s=60, zorder=3, marker='o', linewidths=0.5)

    # Plot explained variance vs. descending power
    ax.plot(indices, relative_explained_variances_sorted_by_power, marker='o', linestyle='-', color='green', linewidth=1.7, markersize=6, label='Descending power')
    ax.scatter(indices, relative_explained_variances_sorted_by_power, color='green', edgecolor='#98FB98', s=60, zorder=3, marker='o', linewidths=0.5)
    ...
    ax.set_ylabel('Relative explained variance', fontsize=14)
    ax.set_title(f'Explained variance vs. increasing $k$ and descending power\n MSE bewteen curves: {mse_between_curves:.3e}', fontsize=16)
...
def plot_partial_fourier_spectra(cumulative_fourier_matrices, full_fourier_transform, plot_indices):
    """Plot partial Fourier spectra (real parts of cumulative Fourier matrices) for selected modes."""
...
# =========================================== DRIVER CODE ============================================
def main(seqid, contact_map_path, mse_output_path, cumulative_power_output_path):
    # Load contact map
    ...
    relative_explained_variances, relative_explained_variances_sorted_by_power, mse_between_curves, \
    approximations, mse_list = reconstruct_full_fft2_from_rfft2(
        ...
        mse_output_path=mse_output_path
    )
    ...
    print(f"Max difference between original fft2 and reconstructed fft2: {max_diff:.5e}")
    ...
    plot_explained_variance(k_list, relative_explained_variances)
    plot_explained_variance_comparison(relative_explained_variances, relative_explained_variances_sorted_by_power, mse_between_curves)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mse_output_path', type=str, required=True, help="Path to save the explained variance MSE value")
    parser.add_argument('--cumulative_power_output_path', type=str, required=True, help="Path to save cumulative powers for seqID")
    ...
    main(
        ...
        mse_output_path=args.mse_output_path,

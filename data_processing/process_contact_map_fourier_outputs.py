...
def main():
    ...
    print(f"Contact map path: {args.contact_map_path}")
    print(f"MSD output path: {args.msd_output_path}")
    print(f"Cumulative power output path: {args.cumulative_power_output_path}")

    os.makedirs(os.path.dirname(args.msd_output_path), exist_ok=True)
    os.makedirs(os.path.dirname(args.cumulative_power_output_path), exist_ok=True)

    # Load contact map
    contact_map_symmetrized, contact_map_symmetrized_normed = get_contact_map(args.contact_map_path)
    ...
    save_cumulative_powers(sorted_unique_indices_below_nyquist, cumulative_powers, args.cumulative_power_output_path)

    # Compute the full FFT2 for comparison
    difference = np.abs(full_fourier_transform - reconstructed_fft2)
    max_diff = np.max(difference)
    print(f"Max difference between full fft2 and reconstructed fft2 using unique modes (should be ~ 0): {max_diff:.5e}")
